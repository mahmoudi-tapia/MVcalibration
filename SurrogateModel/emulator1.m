...
% This script trains an emulator for the physics-based model using some
% simulation data.
...
PARAM1 = 50000;   % Number of iterations for MCMC
...
% Save the plot file
graphName=[ '.\CodeOutput\' Date(1:10) '_' Date(12:13) Date(15:16) '_rHist'  ];
...
% temp1=reshape(A_valid, length(trainingData)^2,1);
% temp2= temp1>0.01;
% fprintf('Number of elements greater than 0.01 in A = %i \n', sum(temp2))
% 
% % Test the mu double star
% [ BetaHAT_valid ] = BetaHAT( A_valid, H, D);
% x_valid = 1.5*xTest
% % x_valid = inputNORM(10,:)'
% 
% ActualOutput = testSample2(x_valid.*( (max(inputRAW)-min(inputRAW))' )+min(inputRAW)')
