...
function [ PredictedOutput , predSD] = emulatorForCrossVal2( iRun, trainingData, testIndex, p, q, PARAM1, PARAM2, PARAM3 )

Date=datestr(now,31);
tic

    
fullData = trainingData;    % Copy the full data

% Leave row i
trainingData(testIndex,:)=[];
...
% Save the plot file
graphName=[ '.\CodeOutput\' Date(1:10) '_' Date(12:13) Date(15:16)  Date(18:19) '_FOLD_' num2str(iRun) '_rHist'  ];
print ('-dtiff', graphName);
close;


%% Validating the emulator with a single data point
% % Use posterior mean as r estiamtes
% r_hat = mean(rSample)'
% A_valid = corrMat(r_hat, inputNORM);
% 
% 
% % Test the mu double star
% [ BetaHAT_valid ] = BetaHAT( A_valid, H, D);
% 
% % Use the omitted data point for validation
% x_valid = fullData(iRun, 1:p)';
% 
% [ mu_valid ] = muDstar( x_valid, BetaHAT_valid, D, A_valid, H, inputNORM, r_hat);
% PredictedOutput = mu_valid.*std(outputRAW)'+mean(outputRAW)';

%% Validating the emulator with a test set (CROSS VALIDATION)
seedNo = 23;
...
% validData = testSet(20, seedNo); % A new test set with seed number 23
% validData = csvread('Data2_MVGP_validation.csv', 1, 0);  % Ignore the file header
validData = fullData(testIndex,:);

% r_hat = mean(rSample)'; % Use the posterior mean as the r estimates
r_hat = rMode; % Use the posterior mode as the r estimates
...
X_validRAW = validData(:,1:p);
% Normalized the input
X_validNORM = bsxfun(@rdivide,bsxfun(@minus,X_validRAW,min(inputRAW)),max(inputRAW)-min(inputRAW));

% Feed the normal input to the function
ActualOutput = validData(:,p+1:p+q);

% Compute the mean vector (predictions) for the T-distribution
[ mu_valid, T_valid ] = muDstar2( X_validNORM, BetaHAT_valid, D, A_valid_chol, H, inputNORM, r_hat);

% Compute the prediction variance
predVAR = cDstar2( X_validNORM, T_valid, A_valid_chol, H, r_hat);
...
predErr = abs(PredictedOutput - ActualOutput);
% predErrPERCENTAGE(i,:) = 100*(PredictedOutput(i,:) - ActualOutput') ./ ActualOutput';

% for i=1:q
%     figure(i)
%     plot(predErr(:,i))
%     grid on
%     graphName=[ '.\CodeOutput\' Date(1:10) '_' Date(12:13) Date(15:16)  '_error_output_' num2str(i) ];
%     print ('-dpng', graphName)
%     close;
% end

% Compute MSE for each output
MSE = mean(sqrt((PredictedOutput-validData(:,p+1:p+q)).^2));
...
Name1=[ '.\CodeOutput\' Date(1:10) '_' Date(12:13) Date(15:16)  Date(18:19) '_FOLD_' num2str(iRun)  '_workspace' ];
save(Name1)
